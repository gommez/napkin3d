...
it("tracks missing inputs and model dimensions without inventing text associations", () => {
  ...
  expect(diagnosticValues(scan, { holeDiametersMm: {} }).unresolved).toEqual(["outer.widthMm", "outer.heightMm", "h.diameterMm", "thicknessMm"]);
  ...
  expect(report.resolution?.dimensions.outerWidth?.origin).toBe("USER_CONFIRMED");
  expect(report.resolution?.dimensions.outerHeight?.origin).toBe("DERIVED");
